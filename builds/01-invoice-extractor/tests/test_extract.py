...
import pytest
from extractor.extract import extract, main

SAMPLES = Path(__file__).resolve().parent.parent / "sample-data"
EXPECTED = {e["file"]: {k: v for k, v in e.items() if k != "file"} for e in json.loads((SAMPLES / "expected.json").read_text())}
...
def test_gives_up_after_retry():
    def always_wrong(text):
        raw = dict(EXPECTED["inv-001.pdf"]); raw["total"] = 1.0; return raw
