...
def test_rejects_mismatched_totals():
    bad = dict(EXPECTED[0]); bad.pop("file"); bad["total"] = bad["total"] + 10
    with pytest.raises(ValidationError, match="total"):
        Invoice(**bad)

def test_rejects_due_before_invoice():
    bad = dict(EXPECTED[0]); bad.pop("file"); bad["due_date"] = "2020-01-01"
    with pytest.raises(ValidationError, match="due_date"):
        Invoice(**bad)

def test_currency_normalised():
    ok = dict(EXPECTED[1]); ok.pop("file"); ok["currency"] = "usd"
