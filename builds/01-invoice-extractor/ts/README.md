...
```bash
npm install
npm test                                                   # 25 vitest tests, fake LLM, no key
...
| `extractor/extract.py` `extract(pdf, llm)`           | `src/extract.ts` `extract(pdfPath, llm = claudeLlm)` | `llm: (text) => Promise<unknown>`; on zod failure the error text is appended to the prompt and the LLM is called once more                                                            |
...
| `tests/` (pytest)                                    | `tests/` (vitest)                                    | Same cases, plus a JSON_SCHEMA parity test and a `ZodError` type test                                                                                                                 |
