...
from .schema import JSON_SCHEMA, MIN_CONFIDENCE, Triage
...
SYSTEM_TEMPLATE = (
    ...
    f"Confidence below {MIN_CONFIDENCE} means needs_human."
)
...
def claude_llm(email: dict, context: str) -> dict:
    ...
    if resp.stop_reason == "refusal":
        return {
            ...
            "confidence": 0.0,
            "summary": "model declined to process this email",
            "urgency": "normal",
            "draft_reply": None,
            "reason": "refusal",
        }
...
def triage_email(email: dict, context: str = DEFAULT_CONTEXT, llm: LLM = claude_llm) -> Triage:
    return Triage(**llm(email, context))
...
def digest(decisions: list[tuple[dict, Triage]]) -> str:
    ...
    drafts = [(e, t) for e, t in decisions if t.draft_reply]
